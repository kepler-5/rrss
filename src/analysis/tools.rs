...
impl Visitor for SimpleStringConstantFolder {
    ...
    fn visit_poetic_number_literal(&mut self, _: &PoeticNumberLiteral) -> walk::Result<Self> {
        Err(WrongType)
    }
    ...
    fn visit_binary_expression(&mut self, _: &BinaryExpression) -> walk::Result<Self> {
        Err(PossibleValueIgnored)
    }
    fn visit_unary_expression(&mut self, _: &UnaryExpression) -> walk::Result<Self> {
        Err(WrongType)
    }
    fn visit_array_subsript(&mut self, _: &ArraySubscript) -> walk::Result<Self> {
        Err(UnknownValue)
